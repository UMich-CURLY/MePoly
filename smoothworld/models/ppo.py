...
class PPO:
        ...
        def __init__(self, policy_class, env, **hyperparameters):
                ...
                self.actor = policy_class(self.obs_dim, self.act_dim * 2).to(self.device)   # ALG STEP 1
        ...
        def learn(self, total_timesteps):
                ...
                t_so_far = self.logger['t_so_far'] + np.sum(batch_lens)

                # Increment the number of iterations
                i_so_far = self.logger['i_so_far'] + 1
                ...
                if self.logger['i_so_far'] % 10 == 0: wandb.log({"advantage_hist": wandb.Histogram(A_k.cpu().numpy())}, step=self.logger['i_so_far'])
                self._log_summary()
        ...
        def get_action(self, obs):
                ...
                # devide to mean and std from the actor output(torch.chunk)
                mean, std = self.actor(obs_tensor).chunk(2, dim=-1)
        ...
        def evaluate(self, batch_obs, batch_acts):
                ...
                mean, std = self.actor(batch_obs).chunk(2, dim=-1)
